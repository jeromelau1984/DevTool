 ...
 *
 */
public final class DlRunnable extends BaseRunnable{

	public static final String TAG = DlRunnable.class.getSimpleName();
	...
	private HashMap<String,DlApk> apkFiles;
...
//		this.isBreak = true;
//		Log.i(TAG, "changeNeedRun isBreak : " + isBreak);
//	}
//	
//	/**
...
//		this.isRunning = true;
//		Log.i(TAG, "enableRun isRunning : " + isRunning);
//	}
//	
...
//	}
	...
	protected DlRunnable(DlListener listener){
	...
	protected void init(DlRemoteFile upgradeZip){
		if(upgradeZip == null || !upgradeZip.isPrepare()){
			if(listener != null) {
				if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion))
					listener.onDlFileError(TAG_INIT_ZIP, cfgVersion , downZipName);
			}	
			return;
		}
		...
		if(upgradeZip.apkFiles == null || upgradeZip.apkFiles.size() == 0){
			if(listener != null){
				if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion))
					listener.onDlFileError(TAG_INIT_APKFILES, cfgVersion , downZipName);
			}
			return;
		}
		
		if(upgradeZip.dlFileName.contains("/")){
			String tmpS[] = upgradeZip.dlFileName.split("/");
			if(tmpS != null && tmpS.length >= 1){
				if(!TextUtils.isEmpty(tmpS[tmpS.length - 1]) && tmpS[tmpS.length - 1].contains(POSTFIX_ZIP))
					this.downZipName = tmpS[tmpS.length - 1].replace(POSTFIX_ZIP, "");
			}
		}else{
			if(!TextUtils.isEmpty(upgradeZip.dlFileName) && upgradeZip.dlFileName.contains(POSTFIX_ZIP)) 
				this.downZipName = upgradeZip.dlFileName.replace(POSTFIX_ZIP, "");
		}
		
		this.cfgVersion = upgradeZip.cfgVersion;
		this.tempUrl = upgradeZip.dlUrl;
		this.needDownloadFilemd5 = upgradeZip.dlFileMd5;
		this.remoteFileSize = upgradeZip.dlFileSize;
		this.apkFiles = upgradeZip.apkFiles;

		Log.v(TAG, "upgradeZip : " + upgradeZip);
		if(TextUtils.isEmpty(mDownloadAbsolutePath)) mDownloadAbsolutePath = "/storage/sdcard0/carrobotUpgrade";
		this.tempFileName = mDownloadAbsolutePath + "/" + downZipName + ".mc";
	...
	protected int download() {
		...
		Log.d(TAG, "downloadZip begin downZipName : " + downZipName);
		Log.d(TAG, "downloadZip begin tempUrl : " + tempUrl);
		Log.d(TAG, "downloadZip begin needDownloadFilemd5 : " + needDownloadFilemd5);
		...
		long remainSize = -1;
		try {
			Log.d(TAG, "downloadZip() URL = " + tempUrl);
			
			if (tempUrl != null && "".equals(tempUrl.trim())) {
				if(listener != null){
					if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion))
						listener.onDlFileError(TAG_EMPTY_URL, cfgVersion , downZipName);
				}
				return DOWNLOAD_CONTINUE;
			}
			if(!TextUtils.isEmpty(tempFileName)){
				File existFile = new File(tempFileName.replace(".mc", POSTFIX_ZIP));
				...
				if(existFile.exists() && !TextUtils.isEmpty(needDownloadFilemd5)){
					...
					Log.i(TAG, "downZipName : " + downZipName);
					...
					if(TextUtils.equals(MD5FileUtil.getMd5ByFile(existFile), needDownloadFilemd5)){
						...
						transactionUnZip("existFile" , downZipName);
						return DOWNLOAD_CONTINUE;
					}
				}
			}
			...
			if (file.exists()) {
				...
				if(remoteFileSize == fileSize){
					...
					transactionUnZip("download code == 416" , downZipName);
					return DOWNLOAD_CONTINUE;
				}
...
//			}
			...
			if (listener != null) {
				if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion)) 
					listener.onProcess(downZipName , cfgVersion , (int) percent, usedTime , (int)remainSize);
			}
				...
				if (length != -1) {
					...
					if (tempFileSize > 0) {
						...
						if (tmpPercent != percent) {
							if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(downZipName)) listener.onProcess(downZipName , cfgVersion , (int) percent, usedTime , (int)remainSize);
						}
						percent = tmpPercent;
					}
				}
	...
	private void reNameTmpFile(String tag , File file) throws IOException{
		...
		File reNameFile = new File(sb.append(downZipName).append(POSTFIX_ZIP).toString());
	...
	private int getDlZipResult(long tempUsedTime, boolean isSuccess,byte[] resultData, File file) throws IOException {
		...
		if (percent != 100) {
			...
			transactionUnZip("download percent 100%" , downZipName);
		}
	...
	private int errorCode(HttpURLConnection conn, int code) {
		...
		usedTime += time;
		if (listener != null) {
			if(!TextUtils.isEmpty(cfgVersion) && !TextUtils.isEmpty(downZipName)) listener.onDlFileError(code, cfgVersion , downZipName);
		}
		return DOWNLOAD_CONTINUE;
	}
...
//	}
	...
	private void transactionUnZip(String tag , String fileName) throws IOException {
		try {
			...
			StringBuffer sb = new StringBuffer(BASE_PATH).append(DL_FILE);
			File reNameFile = new File(sb.append(fileName).append(POSTFIX_ZIP).toString());
			if (FileUtil.isZipFile(reNameFile.getAbsoluteFile()) && TextUtils.equals(needDownloadFilemd5,MD5FileUtil.getMd5ByFile(reNameFile))) {
				...
				if (destFile.exists()) {
					...
					if (null == subFile || subFile.length == 0) {
						unZipLock.release();
						if (null != listener){
							if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion)) listener.onDlFileError(TAG_TRANS_UNZIP , cfgVersion, downZipName);
						}
						return;
					}
					...
					if (apkFiles != null && apkFiles.size() > 0) {
						...
						if (md5EqualCount == apkFiles.size()) {
							...
							if (null != listener){
								if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion))
									listener.onUnzipFinished(true, cfgVersion,downZipName);
							} 
							return;
						} else {
							unZipLock.release();
							if (null != listener){
								if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion))
									listener.onDlFileError(TAG_SIZE_NQ , cfgVersion,downZipName);
							}
							return;
						}
					}
			...
			if(null != listener) {
				if(!TextUtils.isEmpty(downZipName) && !TextUtils.isEmpty(cfgVersion))
					listener.onDlFileError(TAG_LOCK_RELEASE , cfgVersion , downZipName);
			}
		}
	}
